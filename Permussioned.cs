...
public static class Permussioned
{
        ...
        PermissionGroupOccurenceMap permissionGroupOccurenceMap)
    {
        int totalPermutationCount = permissionSetMap
            ...
            .Sum(x => permissionGroupOccurenceMap[x].Count);
        ...
        foreach (var (psId, pgIds) in permissionSetMap)
        {
            foreach (var psId2 in
                     pgIds.SelectMany(x => permissionGroupOccurenceMap[x])
                         .Distinct())
            {
        ...
        PermissionGroupOccurenceMap permissionGroupOccurenceMap)
    {
        ...
        Parallel.ForEach(
            ...
            {
                foreach (var psId2 in
                         permissionSetMap[psId].SelectMany(x => permissionGroupOccurenceMap[x])
                             .Distinct())
                {
            ...
            while (--i >= 0)
            {
                ...
                while (--j >= 0)
                {
                    totalPermutationCount +=
                        permissionGroupOccurenceMap[pgIds[j]].Count;
                }
